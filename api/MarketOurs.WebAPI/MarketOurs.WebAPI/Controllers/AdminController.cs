    ...
    IAdminService adminService) : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<UserDto>>> UpdateUserStatus(string id,
        [FromBody] UpdateUserStatusRequest request)
    {
        ...
        return Ok(user == null
            ? ApiResponse<UserDto>.Fail(404, "用户不存在")
            : ApiResponse<UserDto>.Success(user, "更新用户状态成功"));
    }
}